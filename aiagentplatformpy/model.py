...
from aiagentplatformpy.exception import AiAgentPlatformInvalidEventError
# from aiagentplatformpy.log import log_debug
...
class AiAgentPlatformModel(BaseModel):
...
class IteratorHTTPResponse(HTTPResponse, Generic[T]):
    def __init__(self, raw_response, data: Iterator[T]):
        super().__init__(raw_response)
        self.data = data


class AsyncIteratorHTTPResponse(HTTPResponse, Generic[T]):
    def __init__(self, raw_response, data: AsyncIterator[T]):
        super().__init__(raw_response)
        self.data = data
...
class ListResponse(HTTPResponse, Generic[T]):
    def __init__(self, raw_response, data: List[T]):
...
class HTTPRequest(AiAgentPlatformModel, Generic[T]):
    method: str
    url: str
        ...
        for field in self._fields:
            if line.startswith(field + ":"):
                if data[field] == "":
                    ...
                    raise AiAgentPlatformInvalidEventError(field, line, self.logid)
        raise AiAgentPlatformInvalidEventError("", line, self.logid)
