...
from aiagentplatformpy.exception import AiAgentPlatformPKCEAuthError, AiAgentPlatformPKCEAuthErrorType
from aiagentplatformpy.model import AiAgentPlatformModel
from aiagentplatformpy.request import Requester
from aiagentplatformpy.util import gen_s256_code_challenge, random_hex, remove_url_trailing_slash


class OAuthToken(AiAgentPlatformModel):
    # The requested access token. The app can use this token to authenticate to the AiAgentPlatform resource.
    access_token: str
...
class DeviceAuthCode(AiAgentPlatformModel):
    # device code
    device_code: str
    # The user code
    user_code: str
...
class ScopeAccountPermission(AiAgentPlatformModel):
    permission_list: List[str]


class ScopeAttributeConstraintConnectorBotChatAttribute(AiAgentPlatformModel):
    bot_id_list: List[str]


class ScopeAttributeConstraint(AiAgentPlatformModel):
    connector_bot_chat_attribute: ScopeAttributeConstraintConnectorBotChatAttribute


class Scope(AiAgentPlatformModel):
...
class OAuthApp(object):
    ...
    def _get_www_base_url(self) -> str:
        ...
        if self._base_url in [AiAgentPlatform_COM_BASE_URL]:
...
class WebOAuthApp(OAuthApp):
    ...
    def __init__(self, client_id: str, client_secret: str, base_url: str = AiAgentPlatform_COM_BASE_URL, www_base_url: str = ""):
        """
        :param client_id:
        :param client_secret:
        :param base_url:
...
class JWTOAuthApp(OAuthApp):
    ...
    def __init__(self, client_id: str, private_key: str, public_key_id: str, base_url: str = AiAgentPlatform_COM_BASE_URL):
        """
        :param client_id:
        :param private_key:
        :param public_key_id:
...
class PKCEOAuthApp(OAuthApp):
    ...
    def __init__(self, client_id: str, base_url: str = AiAgentPlatform_COM_BASE_URL, www_base_url: str = ""):
        self._token = ""
        self._requester = Requester()
        super().__init__(
            client_id,
...
class DeviceOAuthApp(OAuthApp):
    ...
    def get_access_token(self, device_code: str, poll: bool = False) -> OAuthToken:
        ...
        while True:
            try:
                return self._get_access_token(device_code)
            except AiAgentPlatformPKCEAuthError as e:
                if e.error == AiAgentPlatformPKCEAuthErrorType.AUTHORIZATION_PENDING:
                    time.sleep(interval)
                    continue
                elif e.error == AiAgentPlatformPKCEAuthErrorType.SLOW_DOWN:
                    if interval < 30:
                        interval += 5
                    time.sleep(interval)
                    continue
...
class CryptoWrapper(object):
    ...
    @staticmethod
    def hmacsha256_and_base64(key_byte, message):
        hash = hmac.new(key_byte, message.encode(), hashlib.sha256)
        hash_in_base64 = base64.b64encode(hash.digest()).decode()
        return hash_in_base64

    @staticmethod
    def sha256_and_base64(body):
        sha256 = hashlib.sha256(body.encode())
        base64_sha256 = base64.b64encode(sha256.digest()).decode()
...
class Signer(object):
    ...
    def sign(self, method, host, uri, headers, body):
        headers['host'] = host
        header_time = headers.get('X-Date')
        if not header_time:
            t = datetime.datetime.utcnow().replace(microsecond=0)
            headers['X-Date'] = t.strftime("%a, %d %b %Y %H:%M:%S GMT")
        if method not in ['PUT', 'PATCH', 'POST']:
            body = ""
        else:
            headers['Digest'] = 'SHA-256=' + CryptoWrapper.sha256_and_base64(body)
        query_string = self.canonical_query_string(uri)
        if query_string:
            uri += '?' + query_string
        path = urllib.parse.quote(uri)
        signed_headers = self.signed_headers(headers)
        string_to_sign = self.string_to_sign(method, uri, headers, signed_headers)
        signature = CryptoWrapper.hmacsha256_and_base64(self.Secret.encode(), string_to_sign)
        auth_header_value = self.auth_header_value(signature, self.Key, signed_headers)
        headers['Authorization'] = auth_header_value
        return headers

    @staticmethod
    def canonical_query_string(uri):
        parsed_uri = urllib.parse.urlparse(uri)
        query = parsed_uri.query
        parsed_query = urllib.parse.parse_qs(query)
        canonical_query = urllib.parse.urlencode(parsed_query, doseq=True)
    ...
    def signed_headers(headers):
        a = list(headers.keys())
        a.sort()
        a.append("request-line")
        return a

    @staticmethod
    def string_to_sign(method, uri, headers, signed_headers):
        str_sign = ''
        for key in signed_headers:
            if key != "request-line":
                value = headers[key]
                str_sign += key + ': ' + value + '\n'
        str_sign += method + ' ' + uri + ' HTTP/1.1'
        return str_sign

    @staticmethod
    def auth_header_value(signature, key, signed_headers):
        return f'hmac username="{key}", algorithm="hmac-sha256", headers="{" ".join(signed_headers)}", signature="{signature}"'
...
class Auth(abc.ABC):
    ...
    def ak_sk_sign(self, method: str, host: str, uri: str, headers: dict, body: dict):
        """
        Construct the authorization header in the http headers.
...
class AppAkskAuth(Auth):
    """
    The fixed access ak sk auth flow.
    """

    def __init__(self, ak: str, sk: str):
        assert isinstance(ak, str) and isinstance(sk, str)
        assert len(ak) > 0 and len(sk) > 0
        self._ak = ak
        self._sk = sk
    ...
    def ak_sk_sign(self, method, host, uri, headers: dict, body: dict) -> None:
        signer = Signer(self.ak, self.sk)
        signer.sign(method, host, uri, headers, json.dumps(body))
